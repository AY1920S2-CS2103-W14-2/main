 ...
 * A supplier is considered unique by comparing using {@code Supplier#isSameSupplier(Supplier)}. As such, adding
 * and updating of suppliers uses Supplier#isSameSupplier(Supplier) for equality so as to ensure that the supplier
 * being added or updated is unique in terms of identity in the UniqueSupplierList. However, the removal of a supplier
 * uses Supplier#equals(Object) so as to ensure that the supplier with exactly the same fields will be removed.
 *
...
public class UniqueSupplierList implements Iterable<Supplier> {
    ...
    public void setSupplier(Supplier target, Supplier editedSupplier) {
        CollectionUtil.requireAllNonNull(target, editedSupplier);
    ...
    public void setSuppliers(List<Supplier> suppliers) {
        CollectionUtil.requireAllNonNull(suppliers);
